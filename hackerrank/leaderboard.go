...
func climbingLeaderboard(ranked []int32, player []int32) []int32 {
	...
	rankMap := make(map[int32]bool)
	for _, score := range ranked {
		if !rankMap[score] {
			uniqueRanked = append(uniqueRanked, score)
			rankMap[score] = true
		}
	}
	...
	newRanks := make([]int32, 0) // Fixed to avoid appending to unused indices

	for _, val := range player {
		rank := int32(1) // Start from the top rank
		for _, score := range uniqueRanked {
			if val >= score {
				break
			}
			rank++
		}
		newRanks = append(newRanks, rank) // Append calculated rank
	}
